...
void breeding(int p_lgth, int c_lgth, int child[p_lgth][c_lgth], const int parent[p_lgth][c_lgth]){
	...
	while(i<p_lgth){
		...
		posx(c_lgth, child[i], parent[i], parent[i + 1], r, index);	//generar primer hijo
		posx(c_lgth, child[i + 1], parent[i + 1], parent[i], r, index);	//invertir roles de padres y generar segundo hijo
		/*imprimir resultados*/
